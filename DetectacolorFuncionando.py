...
while(True):
    ...
    if valido:
       ...
       #Guardamos el rango de colores hsv (negros) 
       #bajos = np.array([0,0,0], dtype=np.uint8)
       #altos = np.array([75,75,75], dtype=np.uint8)

       #Guardamos el rango de colores hsv (Rojos) 
       bajos = np.array([134,60,140], dtype=np.uint8)
       altos = np.array([245,255,210], dtype=np.uint8)
       ...
       for extension in areas:
           if extension > 600:
               ...
               if len(approx)==3:
                   ...
                   #colo y tipo de letra de leyenda 
